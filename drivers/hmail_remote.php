...
/**
 * hMailserver remote external accounts driver
 *
 * @version 1.0
...
class rcube_hmail_remote_externalaccounts
{
    ...
    private function data_handler($data)
    {
        ...
        $hmailRemoteUrl = $rcmail->config->get('hms_externalaccounts_remote_url', false);
        if (!$hmailRemoteUrl) {
            rcube::write_log('errors', 'Plugin hms_externalaccounts (hmail remote driver): $config[\'hms_externalaccounts_remote_url\'] is not defined.');
            return HMS_ERROR;
        }
        ...
        elseif($result['error'] != HMS_SUCCESS) {
            rcube::write_log('errors', 'Plugin hms_externalaccounts (hmail remote driver): ' . $result['text']);
            return $result['error'];
        }
